...
describe("Test all functions in the lib folder",()=>{
    describe("Test all functions that are not middlewares",()=>{
        ...
        describe("generate Token", ()=>{
            ...
            let crypto = {
              randomBytes: jest.fn((byteLength, callback) => { return callback(null , randomString )}),
            };
        ...
        it("should throw reject with an error if crypto fails", ()=>{
            ...
            let crypto = {
                randomBytes: jest.fn((byteLength, callback)=> { return callback(new Error(errorMessage)) })
            }
            expect(resetTokens.generateToken(crypto)).rejects.toEqual(Error(errorMessage));
        })
    ...
    describe("Test all middlewares that do not interact with DB", () =>{
        ...
        describe("hash password",()=>{
            ...
            it("should call next with custom ApiError if password and email is absent", ()=>{
                ...
                expect(next).toHaveBeenCalledWith(ApiError.incorrectCredentials());
                expect(ApiError.incorrectCredentials).toHaveBeenCalled();
                expect(ApiError.incorrectCredentials).toHaveBeenCalledWith(errorMessage);
            });
            ...
            it("should call next with an Error if error occurs during hashing", ()=>{
                ...
                let bcrypt = {
                    hash:jest.fn(( password, salt, callback ) => { return callback(intentionalError) })};
                let samplePassword = "samplePassword";
                let sampleEmail = "sampleEmail";
                req.body.password = samplePassword;
                req.body.email = sampleEmail;
        ...
        describe("reset password", ()=>{
            describe("verify Token", ()=>{
                ...
                it("should return a promise ",  ()=>{
                    const returnedPromise = VerifyToken(token,clientToken,next,bcrypt,ApiError);
                ...
                it("should reject with an error if bcrypt compare fails", ()=>{
                   ...
                   let bcrypt  = {
                    compare: jest.fn((clientToken, token, callback  )=>{ return callback(new TypeError) })
                    };
                    expect(VerifyToken(
                      token,
                      clientToken,
            ...
            describe("hash new password",()=>{
                ...
                let bcrypt = { 
                    hash:jest.fn((password, salt = 10, callback ) => { return callback(null, hashedPasswordResult)  })
                }
                ...
                it ("should reject with an error if bcrypt hash fails",()=>{
                    let bcrypt = {
                        hash:jest.fn((password, salt=10, callback ) =>{ return callback( new Error(customErrorMessage) )})
                    }
                    let params = [
                      "clientPassword",
                      true,
                    ...
                    expect(HashNewPassword(...params)).rejects.toEqual(Error(customErrorMessage))
                });
                ...
                it("should return hashed function if bcrypt hash passed", ()=>{
                    ...
                    expect(HashNewPassword(...params)).resolves.toEqual(hashedPasswordResult);
                });
